...
flags.DEFINE_bool(
    "subsample", False,
    "If set, Frames will be subsampled for DBoF model.")
...
class LstmFixedModel(models.BaseModel):
  ...
  def create_model(self, model_input, vocab_size, num_frames, **unused_params):
    ...
    #
    # stacked_lstm = tf.contrib.rnn.MultiRNNCell([
    #     tf.contrib.cudnn_rnn.CudnnCompatibleGRUCell(lstm_size)
    #     for _ in range(number_of_layers)
    # ])
    #
    #
    # outputs, state = tf.nn.dynamic_rnn(
    #     stacked_lstm, model_input,  sequence_length=model_input.shape[1].value, dtype=tf.float32)  # ,
    #
    # aggregated_model = getattr(video_level_models,
    #                            FLAGS.video_level_classifier_model)
    #
